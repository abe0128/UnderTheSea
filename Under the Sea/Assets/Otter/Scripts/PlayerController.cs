...
public class PlayerController : MonoBehaviour {
    public float jumpVelocity;                  // Changeable value, jump velocity
    public float swimVelocity;                  // Changeable value, swim velocity

    private float stamina;                      // To hold the player's stamina
    private float movement_speed;               // To hold the player's movement speed
    ...
    private EnvironmentState currentEnvState;   // To hold the current state of the player

    // State machine: Holds two environmental states
    ...
    void Start() {
        ...
        currentEnvState = EnvironmentState.OnLand;                      // set currentState to onLand
        movement_speed = 0.1f;                                          // set movement speed to 0.1f for land
    ...
    private void Update() {
        ...
        // Switch Statement: Check which current state the player is in
        switch (currentEnvState)
        {
            // If case is OnLand, do the following
            case EnvironmentState.OnLand:
                PlayerMovement();                   // Calls PlayerMovement function, allows basic movement (left to right)
                PlayerJump();                       // Calls PlayerJump function, allows player to jump
                movement_speed = 0.1f;              // Set player's movement modifier to 0.1f (normal speed on land)
                GetComponent<Rigidbody>().drag = 0; // Set player's drag to zero, from Ridigbody component
                break;
            // If case is InWater, do the following
            case EnvironmentState.InWater:
                ...
                movement_speed = 0.05f;             // Set player's movement modifier to 0.05f (slower movement in water)
    ...
    /// <summary>
    /// Player's OnTriggerStay commands
    /// Water: When player stays in water, set EnvironentState to InWater.
    /// </summary>
    /// <param name="other"></param>
    private void OnTriggerStay(Collider other)
    {
        // When onTrigger is tag Water
        if (other.gameObject.tag == "Water")
        {
            currentEnvState = EnvironmentState.InWater; // Change state to InWater
        }
    }

    /// <summary>
    /// Players OnTriggerExit commands
    /// Water: When player leaves water, set EnvironmentState to OnLand
    /// </summary>
    /// <param name="other"></param>
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.tag == "Water")
        {
            currentEnvState = EnvironmentState.OnLand;  // Change state to OnLand
        }
    }
    ...
    private void PlayerJump()
    {
        ...
        {
            GetComponent<Rigidbody>().velocity = Vector3.up * jumpVelocity; // Player Jump, Uses Rigidbody, set for player's vector to move up * jumpvelocity
            stamina -= 2;                                                   // Reduce stamina uppon jumping physics
        }
    }
    ...
    private void PlayerSwim()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Space) && boost_offset < 60)
        {
        ...
        else if (Input.GetKeyUp(KeyCode.Space) && boost_offset >= 60)
        {
        ...
        //Debug.Log("Boost Offset: " + boost_offset);
    }
